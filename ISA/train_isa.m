...
function [] = train_isa(network, X, bases_path, params)
...
if params.layer ==2
    ...
    X = single(transactConvISA(single(X), network.isa{1}, network.isa{2}, params.postact.layer1));
...
if params.layer ==3
    ...
    [V_l1l2, ~, ~] = pca(act_l1_pool_list);
end
%%%%%% quick fix
params.pca_dim = min(size(V,1), params.pca_dim);
%%%%%%%%%%%%%%%%
